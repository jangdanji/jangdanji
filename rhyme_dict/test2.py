...
class KoreanToRoman(object):
    ...
    def __init__(self, unravel=False):
    ...
    def transcribe(self, word, special=False, separator=False, capital=False, transliterate=False) -> str:
        ...
        # 낱자를 자모로 분해
        for c in word:
            if ord(c) >= 0xAC00 and ord(c) <= 0xDC73:
        ...
        if self.unravel:
            if separator:
                ...
                self.transcribed = ''.join(syllables)
            return pre + self.transcribed + post
        else:
            # 음절 구분 기호 삽입
            self.transcribed = '/'.join(syllables)
...
class EnglishToHangul(object):
    ...
    def transcribe(self, word, special=False, phonetic=False, derivative=False, debug=False) -> str:

        pre, pure_word, post = '', '', ''
        for l in word:
            if (ord(l) >= 0x41 and ord(l) <= 0x5A) or (ord(l) >= 0x61 and ord(l) <= 0x7A) or (ord(l) == 0x27):
                pure_word += l
            elif special:
                if l == word[0]:
                    pre = l
...
def arg_parse() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action = 'store_true',
        default = False,
        help = 'Keep special characters in the output. Like parentheses, one character each before and after the given word is kept, and others are ignored.'
    )
    parser.add_argument(
        '-c',
        dest = 'capital',
...
if __name__ == '__main__':
    ...
    K2R = KoreanToRoman(unravel=args.unravel)
    E2H = EnglishToHangul()
    ...
    for phrase in args.words:
        ...
        output.clear()
        for word in words:
            if p.match(word[0]) or p.match(word[1]):
                output.append(E2H.transcribe(word, special=args.special, phonetic=args.phonetic, derivative=args.derivative, debug=args.debug))
            else:
                output.append(K2R.transcribe(word, special=args.special, separator=args.separator, capital=args.capital, transliterate=args.transliterate))
        print(' '.join(output))
